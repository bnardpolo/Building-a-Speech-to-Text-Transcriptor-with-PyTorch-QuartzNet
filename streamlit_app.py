# streamlit_app.py — Local QuartzNet (.pt or .nemo) + Whisper, optional WER/CER + BLEU + translate
...
from test import QuartzNetTester, save_result  # save_result writes WER/CER JSON in outputs/results
...
RESULTS_DIR = ROOT / "results"   # your repo already has /results; keep also at root for convenience
for d in (OUT, AUDIO_DIR, DOC_DIR, TRANS_DIR, RESULTS_DIR):
...
# OpenAI client (needs OPENAI_API_KEY in env for Whisper + translate)
client = OpenAI()
...
def compute_bleu(reference: str, hypothesis: str) -> Optional[float]:
    ...
    try:
        return float(sentence_bleu([ref], hyp, smoothing_function=smooth))
    except Exception:
        return None
...
with left:
    ...
    ckpt = st.text_input("QuartzNet checkpoint", value=QUARTZNET_CKPT if engine == "Local QuartzNet" else "")
...
with main:
    ...
    up_media = st.file_uploader("Upload audio or video (optional)")
    ...
    if st.button("Run Pipeline", use_container_width=True):
        ...
        try:
            ...
            # If we have a WAV, do ASR
            if wav_path is not None:
                if engine == "OpenAI Whisper":
                    text_source = transcribe_audio_wav_openai(wav_path)
                    model_name = "whisper-1"
                else:
                    ckpt_path = (ckpt or "").strip()
                    if not ckpt_path:
                        st.error("QuartzNet checkpoint not provided.")
                        st.stop()
                    if ckpt_path.lower().endswith(".nemo"):
                        # ---- Option A: use NeMo for .nemo ----
                        try:
                            ...
                            st.error(
                                ...
                                "pip install -U nemo_toolkit[asr] Cython youtokentome==1.0.6"
                            )
                            st.stop()
                        with st.spinner("Loading NeMo QuartzNet (.nemo)…"):
                            asr = EncDecCTCModel.restore_from(ckpt_path, map_location="cpu")
                            asr.eval()
                        with st.spinner("Transcribing with NeMo…"):
                            text_source = asr.transcribe(paths2audio_files=[str(wav_path)])[0]
                        model_name = "QuartzNet (NeMo .nemo)"
                    else:
                        # ---- Your local tester for .pt/.pth ----
                        proc = AudioProcessor(sample_rate=sr)
                        tester = QuartzNetTester(ckpt_path, proc)
                        text_source = tester.transcribe(str(wav_path))
                        model_name = "QuartzNet (.pt)"
            ...
            # Optional WER/CER + BLEU + save JSON
            metrics = {}
            out_json_path = None
            if ref_text.strip() and (wav_path is not None or text_source.strip()):
                # WER/CER via your existing helper
                out_json_path, metrics = save_result(
                    str(wav_path) if wav_path is not None else "",
                    ref_text,
                    text_source,
                    model_name=model_name or used_engine
                )

            bleu = compute_bleu(ref_text, text_source) if ref_text.strip() else None
            runtime_s = round(time.time() - t0, 3)

            # If save_result wrote JSON, append extra fields you asked for
            if out_json_path:
                try:
                    ...
                    st.info(f"Saved metrics JSON: {p}")
                except Exception as e:
                    st.warning(f"Could not update results JSON with extras: {e}")
            ...
            # Show quick metrics summary on screen
            if metrics:
                st.write({
                    "WER_raw":  metrics.get("WER_raw"),
                    "CER_raw":  metrics.get("CER_raw"),
                    "WER_norm": metrics.get("WER_norm"),
                    "CER_norm": metrics.get("CER_norm"),
                    "BLEU": bleu,
                    "runtime_s": runtime_s,
                    "engine": used_engine,
                    "model_name": model_name or used_engine,
